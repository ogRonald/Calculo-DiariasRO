...
export default function CalculadoraDiarias() {
  ...
  useEffect(() => {
    if (usuario) {
      ...
      setFormData((atual) => ({ ...atual, servidor: usuario.nomeCompleto || atual.servidor, matricula: usuario.matricula, orgao: usuario.orgao }));
    }
  }, [usuario]);
  ...
  const handleSalvarHistorico = async () => {
    ...
    try {
      const salvo = await salvarHistorico({
        usuarioId: usuario.id, destino: formData.destino,
        dataHoraInicio: formData.dataHoraInicio, dataHoraFim: formData.dataHoraFim,
        totalDias: resultadoCalculo.totalDias, valorUnitarioBrl: resultadoCalculo.valorUnitarioBaseBrl,
        valorTotalBrl: resultadoCalculo.valorTotalBrl
      });
      setHistorico((atual) => [salvo, ...atual]);
      setHistoricoSalvo(true);
      setActiveTab('history');
    } catch (erro: any) { setErroValidacao(erro.message || 'Não foi possível salvar no histórico.'); }
  };
  ...
  return (
    ...
    <div className="relative min-h-screen text-slate-800 font-sans">
      ...
      {/* Imagem de Fundo (usando fixed para preencher a tela mesmo com scroll) */}
      <div 
        className="fixed inset-0 z-0 bg-cover bg-center bg-no-repeat"
        style={{ backgroundImage: "url('/background_login.jpg')" }}
      />
      
      {/* Overlay escuro (usando fixed também) - Aqui deixei um pouco mais transparente (40%) para não escurecer demais a tela toda */}
      <div className="fixed inset-0 z-0 bg-slate-900/40" />

      {/* Adicionado relative e z-10 no header para ele ficar por cima do fundo */}
      <header className="relative z-10 bg-[#0F2C59] text-white p-4 shadow-md border-b-4 border-[#059669]">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => router.push('/perfil?editar=true')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  readOnly
                  className="w-full p-2.5 border border-slate-300 rounded-md bg-slate-50 text-slate-600 focus:outline-none"
                />
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead><tr className="border-b bg-slate-100 text-xs text-slate-600 uppercase">
                  <th className="p-3">Servidor</th><th className="p-3">Destino</th><th className="p-3">Qtd.</th><th className="p-3 text-right">Total</th><th className="p-3 text-center">Status</th>
                </tr></thead>
                <tbody className="divide-y text-sm">
                  {historico.map((item) => <tr key={item.id}>
                    <td className="p-3 font-medium">{usuario?.nomeCompleto}</td>
